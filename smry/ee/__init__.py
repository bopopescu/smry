#!/usr/bin/env python
"""
Initialize the Earth Engine API Package

Revised to interface with Django framework.
...
#  smrylogger.debug("credentials: %s", credentials)
#  smrylogger.debug("opt_url: %s", opt_url)
 
  data.initialize(credentials, (opt_url + '/api' if opt_url else None), opt_url)
#   smrylogger.debug("data.initialize() complete")
  # Initialize the dynamically loaded functions on the objects that want them.
  ApiFunction.initialize()
#   smrylogger.debug("Apifunction initialized")
  Element.initialize()
  Image.initialize()
  Feature.initialize()
  ...
  Terrain.initialize()
#   smrylogger.debug("Functions initialized")
...
#     smrylogger.debug("tokens: %s", tokens)
...
def _ResetGeneratedClasses():
  ...
  for name in _generatedClasses:
    ...
    del globals()[name]
  _generatedClasses = []
  # Warning: we are passing all of globals() into registerClasses.
...
def ServiceAccountCredentials(email, key_file=None, key_data=None):
  ...
  Revised for use within an Apache2 server environment.
    W. R. Emanuel, University of Maryland, College Park
    E-Mail: wemanuel@umd.edu
    Gmail: wemanuel@gmail.com
  
  Expect the key file to reside in the directory that imports the ee package.
  """
#  smrylogger.debug("ee/ServiceAccountCredentials( )")
  
  if key_file:
    # Get the current directory.
    module_dir = os.path.dirname(__file__)
    
    # Revise module_dir to parent directory.
    vals = module_dir.split('/')
    vals = vals[1 : -1]
    module_dir = ''
    for x in vals:
        module_dir += ('/' + x)
#     smrylogger.debug("module_dir: %s", module_dir)

    # Create path to key_file.
    file_path = os.path.join(module_dir, key_file)
#    smrylogger.debug("file_path: %s", file_path)

    # Read the key file with full path.
